setwd("C:\\Users\\Rui Sarmento\\Documents\\Trabalho\\SimRank")
...
graph <- read.csv("Relationships4-R.txt", sep=" ",header=FALSE)
...
#graph <- read.csv("Tests.txt", sep="\t",header=FALSE)
...
df_simrank <- similarity(relations=list_relations$relations, sim_df = list_relations$sim_df, sim_df_old = list_relations$sim_df_old)
}
...
return (list(relations=relations,sim_df=sim_df, sim_df_old=sim_df_old))
}

niter <- 50
similarity <- function(relations, sim_df, sim_df_old){
